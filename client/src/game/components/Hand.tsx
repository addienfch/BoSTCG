...
const Hand = ({ position = [0, 0, 0] }: HandProps) => {
  ...
  useEffect(() => {
    ...
    for (let index = 0; index < playerHand.length; index++) {
      ...
      // Move cards far up in frame for maximum visibility with pulled-back camera
      const y = -Math.abs(normalizedPos) * arcHeight + 0.3; // Much higher for clear visibility 
      
      // Curve the hand toward the camera for better visibility - extreme curve for visibility
      const z = 0.5 - Math.abs(normalizedPos) * 0.3; // Much closer to camera view
